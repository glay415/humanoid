...
- **[추적 대상] episodic 계측 + 연속성 confound + auto-encode 미발화(T2)**: (a) 계측은 P1 으로 수정(vector_db.collection.count()). (b) **P2(C1 재실행)에서 episodic=0 전 23턴 — 확정**: 멀티세션 연속성은 episodic 아닌 LLM dialogue 컨텍스트(추측 아닌 0 입증). (c) **T2b 진단 완료 → ADR-048**: ADR-046 부작용 가설 **기각**(코드+측정: 트리거는 emotion_appraisal LLM 의 *사용자감정* 평가, 압축상태 무관 — 자기 가설 철회·검증). 진짜 원인 = **기존 결함**: `auto_encoding_threshold=1.2` 가 과도. 측정(`t2b_intensity_probe`): 현실 대화 최강 발화만 겨우 1.2, 평상 토로 0.5~0.9 → **episodic auto-encode 평상시 死 = 일화기억/재고정화 pillar 실대화 미작동**(C1 연속성 = 순수 LLM 컨텍스트, 이중확인). 북극성 관점 ADR-046 포화보다 치명. 임계 재조정 = 사용자 calibration 체크포인트(ADR-048, P3 와 함께 결정). ADR-045/046/048 참조.
