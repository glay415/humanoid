...
## ADR-046 — 상태 포화 수정: A 입력항 압축(soft ceiling) (2026-05-19)

**Context**: C1 dogfooding(`c1_relationship_sim.md`) — 사람 평결 Q3
"INTJ 건조함이 세션 2부터 깨짐", Q4 봇-순간=억지 클레버. 동시점에
측정된 상태 포화(bonding/arousal→1.0). T1 코드+실측 진단:

- `InternalState.update`: Δ = A·exp + W·dev + D·(base−state), [0,1]
  hard clamp. bonding A행 = social_reward 단독 게인 **0.3**, 회복은 D
  대각 **0.1**. *지속 동방향 입력*(현실적 긴 지지 대화 = 매 턴 高
  social_reward)에서 push(+0.24~0.30) ≫ D 복원(포화 시 −0.06).
- LLM-free 결정론 재현: intj 에 social_reward=0.9 지속 입력 →
  bonding 0.40→0.71(t2)→0.99(t4)→**1.0 고정**, arousal 동형, stress
  →1.0(t12). C1 의 세션2 포화를 그대로 재현.
- **felt(Q3) ↔ 측정(포화) ↔ 코드(A≫D 비대칭) 삼자 일치.** 포화된
  "최대 유대·각성"이 affect_translator 에 주입 → INTJ register 붕괴.

근본: W−D 안정성 증명(`validate_stability`, J=W−D 고유값<0)은 *무입력
시* baseline 감쇠만 보장 — *지속 입력 하 dynamic-range* 는 그 증명의
범위 밖. 천장 clamp = 9-dim 내면이 변별력 상실.

**Decision**: 옵션 1 — **A 입력항만 압축(soft ceiling)**. 사용자 승인.
- `update()`/`compute_decomposition()` 에서 `a_term = A·exp` 의 각
  성분에 헤드룸 곱: 양(+) 성분 ×`(1−state)`, 음(−) 성분 ×`state`.
  → state 가 [0,1] 경계로 *점근*(clamp 안 침), 입력만으로 천장 못 감.
  W·D·Δmax·reactivity·최종 [0,1] safety clip *전부 불변*.
- **안정성 증명 무영향**: W,D 미변경 → J=W−D 고유값 그대로 →
  `validate_stability` 빌드 assert 통과. 증명이 다루던 *무입력 동역학*
  은 정확히 불변; 우리는 증명이 *다룬 적 없는* 비동차 입력항만 재형상
  (그게 T1 발견의 정확한 핵심).

### 명시적 비범위 / 리스크

- `apply_fast_path`(직접 state nudge)는 입력 ratchet 경로 아님 → 미변경.
- *의도된 동역학 변경*이라 값-고정(exact-value) 단위/시나리오/W-
  sensitivity/lifecycle 테스트 일부가 *깨질 것* — 정상. triage 원칙:
  (a) 값-고정 shift = 의도, 기대값 갱신 / (b) 불변식(stability·[0,1]
  bound·시나리오 emergent invariant) 위반 = 수정 잘못, 롤백/재설계.
  테스트를 그냥 통과하게 고치지 않는다(원칙 vs 일화).
- 수용 기준: 지속 강입력에도 bonding/arousal < ~0.98(천장 회피) +
  반대 입력에 여전히 끌려옴(dynamic range 보존). C1 합성 재실행으로
  세션2 포화 소멸 확인. felt 재평가는 별도(사람) — 본 ADR 은 *기계적
  포화 제거*까지.

### 구현·회귀 결과 (2026-05-19)

`InternalState.update`+`compute_decomposition` 압축입력 구현. 수용
테스트 `test_state_saturation.py`(+3) red→green: 수정 전 bonding
1.0 clamp → 후 15턴 지속입력에도 **0.82**(점근), dynamic range·
[0,1]·W−D 안정성(`validate_stability`) 전부 보존.

전체 회귀 triage: **1033 passed + 7 failed**. 7 = `test_group3_self_
existence.py`(시나리오20–25) + `test_main_cli::...wires_all_dependencies`
— state-of-the-project 에 *이미 명시된* chromadb 병렬 flake 세트와
정확히 일치. **추측 배제, isolation 재실행으로 검증 → 8 passed 0
fail(전부 PASS)**. 즉 불변식 위반 0, 값-고정 깨짐 0(아키텍처 테스트가
불변식 기반이라 압축입력 blast-radius 최소 — option 1 선정 근거 입증).
실효 baseline 1037→**1040 flake-free**.

triage 원칙 적용 결과: "테스트를 통과시키려 무르게 고친" 것 0건 —
실패가 전부 환경 flake 였고 코드 무관임을 *재현으로* 확정.

**Status**: accepted (구현·전체 회귀 clean 완료). C1(dogfooding) →
felt 증상 → 측정 포화 → 코드 근본 → 실측 확인 → 최소 코어 수정 →
회귀 flake-only 의 완결. 드리프트 비판의 귀결: 측정 인프라가
인지아키텍처의 실 결함을 잡아 고친 *첫 사례* + 이 arc 의 *첫 product
코드 변경*. 잔여: T2(episodic 계측+confound)/slice-2b/I8 기질-조건부
는 별개 트랙. felt 재평가(C1 파이프라인 재실행)는 사람 몫 후속.
