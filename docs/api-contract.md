...
### `POST /api/instances/{id}/undo` — ADR-034

직전 conversation turn 1개를 in-memory 상태에서 되돌린다. 인스턴스 내부의 3턴 ring buffer (`Orchestrator._undo_stack`) 가 turn 시작 직전 snapshot 을 보관 — 최대 3회 연속 undo 가능. 4번째 호출은 `400`.

복원 대상: 9-dim `internal_state` / `mood` / `raw_core_affect` / `drives` / `temperament.baselines` / `self_model` / `other_model` / `metacognition` (resource·confidence·goal_progress) / `dmn.unappraised_queue` / `dialogue_buffer` / `turn_number` / `markers` / `fast_path.patterns` / `_instance_mood_history` 끝 항목.

의도된 한계: `vector_db` (`auto_encode`), `dmn_artifacts.sqlite` (append-only), `turns.jsonl` 로거는 보존 — 감사·재시작 안정성 우선. 자세한 결정은 `docs/decisions.md` ADR-034.
...
{
  instance_id: string,
  undone_turn: number,      // 되돌려진 turn 번호
  turn_number: number,      // 복원 후 turn_number
  remaining_undos: number,  // 남은 buffer 크기 (0~2)
}

// 400 — buffer 비었음 ("nothing to undo")
// 404 — 인스턴스 없음
```
