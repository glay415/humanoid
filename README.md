...
오프라인 분석은 `scripts/analyze.py` (Wave 14B). `uv sync --extra analyze`
(또는 `pip install -e .[analyze]`) 로 pandas + matplotlib 를 켜고
`python scripts/analyze.py <instance_id> --charts ./reports/<id>` 또는
`--json --out report.json` 으로 호출한다.
