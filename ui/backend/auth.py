"""보안 헬퍼 — env 기반 CORS / admin token 구성.
...
이 모듈은 `ui.backend.app` 의 setup 단계에서만 호출된다. 환경변수:
  ...
  HUMANOID_ADMIN_TOKEN       선택. 설정되면 destructive 라우트가 헤더 검사.
                             production 에서는 필수.
...
ADMIN_TOKEN_VAR = "HUMANOID_ADMIN_TOKEN"
...
def configured_admin_token() -> str | None:
    """현재 설정된 admin token (없으면 None).

    production 에서 토큰 미지정이면 RuntimeError 을 던지는 책임은
    `enforce_production_invariants()` 에 있다. 여기서는 단순 조회만.
    """
    raw = os.environ.get(ADMIN_TOKEN_VAR)
    if raw is None:
        return None
    raw = raw.strip()
    return raw or None
...
def admin_token_required() -> bool:
    """현재 env 에서 destructive 라우트가 토큰을 요구하는가."""
    return configured_admin_token() is not None


def verify_admin_token(header_value: str | None) -> bool:
    """주어진 헤더 값이 설정된 토큰과 일치하는지. 토큰 미설정이면 항상 True
    (dev 자유 모드 유지). 일치 비교는 단순 문자열 비교 — timing 공격 방어가
    필요해지면 `secrets.compare_digest` 로 교체."""
...
def enforce_production_invariants() -> None:
    ...
    - HUMANOID_ADMIN_TOKEN 이 비어 있으면 raise.
    """
    ...
    if configured_admin_token() is None:
        raise RuntimeError(
            f"must set {ADMIN_TOKEN_VAR} in production"
        )
...
CORS_ALLOW_HEADERS: tuple[str, ...] = ("Content-Type", ADMIN_TOKEN_HEADER)
